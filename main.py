...
try:
    ...
    st.error("🚨 API 키가 설정되지 않았습니다. GitHub Secrets를 확인해주세요.")
    st.stop()
...
def summarize_with_gemini(comments_list, video_title):
    ...
    try:
        ...
        # 429 에러 방지: 좋아요 순으로 상위 50개만 추출하여 요약
        sorted_comments = sorted(comments_list, key=lambda x: x["좋아요"], reverse=True)
        sample = sorted_comments[:50]
        text = "\n".join([f"- {c['댓글']}" for c in sample])
        
        prompt = f"""다음은 유튜브 영상 '{video_title}'의 베스트 댓글(좋아요 상위 50개)들이야.
...
with st.sidebar:
    ...
    st.markdown("##### 🌐 번역 설정")
    lang_options = {'한국어': 'ko', '영어': 'en', '일본어': 'ja', '중국어': 'zh-CN', '원문 유지': 'none'}
    selected_lang_name = st.selectbox("목표 언어", list(lang_options.keys()))
...
if search_button:
    ...
    # 번역 로직 추가 (수집 직후)
    if comments and target_lang_code != 'none':
        with st.spinner(f"🌐 댓글을 {selected_lang_name}로 번역하는 중..."):
            for c in comments:
                c["댓글"] = smart_translate(c["댓글"], target_lang=target_lang_code)

    st.session_state["comments"] = comments

    if GEMINI_API_KEY and GEMINI_AVAILABLE and comments:
        with st.spinner("🤖 Gemini AI가 베스트 댓글을 요약하는 중... (Top 50)"):
            summary = summarize_with_gemini(comments, video_info["title"])
...
if "video_info" in st.session_state and "comments" in st.session_state:
    ...
    if "summary" in st.session_state and st.session_state["summary"]:
        ...
        st.markdown(
            f'<div class="ai-card">'
            f'<div class="ai-title">🤖 Gemini AI 베스트 댓글 요약</div>'
    ...
    with tab3:
        ...
        avg_len = sum(len(str(c["댓글"])) for c in comments) / total_c if total_c else 0
        ...
        short_c = sum(1 for c in comments if len(str(c["댓글"])) <= 30)
        pct_short = (short_c / total_c * 100) if total_c else 0

        long_c = sum(1 for c in comments if len(str(c["댓글"])) > 100)
        ...
        for i, c in enumerate(top5):
            safe_t = str(c["댓글"]).replace("<", "&lt;").replace(">", "&gt;")
            safe_a = str(c["작성자"]).replace("<", "&lt;").replace(">", "&gt;")
            st.markdown(
